...
export default async function EmailPage() {
  ...
  return <section className="admin-section"><div className="container"><h1 className="admin-title">Email delivery</h1><p className="admin-sub">Emails waiting to be sent.</p><div className="table-wrap"><table className="table"><thead><tr><th>Created</th><th>Recipient</th><th>Subject</th><th>Tries</th><th></th></tr></thead><tbody>{records.map((record) => <tr key={record.id}><td>{formatDateTime(record.createdAt)}</td><td>{record.to}</td><td><strong>{record.subject}</strong>{record.lastError ? <div className="form-error" style={{ marginTop: 6 }}>{record.lastError}</div> : null}</td><td>{record.attempts ?? 0}</td><td><EmailRetryButton id={record.id} /></td></tr>)}</tbody></table></div>{records.length ? null : <div className="notice" style={{ padding: "40px 0" }}><h1>All emails sent</h1></div>}</div></section>;
}
