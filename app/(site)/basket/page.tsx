...
export default async function BasketPage() {
  ...
  const banquestEnvironment = process.env.BANQUEST_ENV?.trim().toLowerCase() === "production" ? "production" : "sandbox";
  const turnstileSiteKey = process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY?.trim() ?? "";
  return <><div className="band"><div className="container" style={{ padding: "52px 40px" }}><div className="page-head"><div className="he he--left" lang="he">רשימת הכיבודים</div><h1>Your sponsorship list</h1><div className="meta">Sponsor several kibbudim with one credit-card payment.</div></div></div></div><section className="container" style={{ padding: "48px 40px 96px" }}><CartCheckoutExperience items={items} tokenizationKey={tokenizationKey} banquestEnvironment={banquestEnvironment} turnstileSiteKey={turnstileSiteKey} /></section></>;
}
